module @module {
  ...
  util.func public @prefill_moe$async(%arg0: !hal.buffer_view, %arg1: !hal.fence, %arg2: !hal.fence) -> !hal.buffer_view attributes {inlining_policy = #util.inline.never, iree.abi.model = "coarse-fences", iree.abi.stub} {
  ...
  util.func public @prefill_moe(%arg0: !hal.buffer_view) -> !hal.buffer_view attributes {iree.abi.stub} {
    %0 = util.null : !hal.fence
    %c-1_i32 = arith.constant -1 : i32
    %c0 = arith.constant 0 : index
    %device_0 = hal.devices.get %c0 : !hal.device
    %fence = hal.fence.create device(%device_0 : !hal.device) flags("None") : !hal.fence
    %1 = util.call @prefill_moe$async(%arg0, %0, %fence) : (!hal.buffer_view, !hal.fence, !hal.fence) -> !hal.buffer_view
    %status = hal.fence.await until([%fence]) timeout_millis(%c-1_i32) : i32
    util.return %1 : !hal.buffer_view
  }
  util.func public @decode_moe$async(%arg0: !hal.buffer_view, %arg1: !hal.fence, %arg2: !hal.fence) -> !hal.buffer_view attributes {inlining_policy = #util.inline.never, iree.abi.model = "coarse-fences", iree.abi.stub} {
    %c0_i64 = arith.constant 0 : i64
    %cst = arith.constant 0.000000e+00 : f32
    %cst_0 = arith.constant 0xFF800000 : f32
    %c8_i64 = arith.constant 8 : i64
    ...
    %extracted_slice_5 = tensor.extract_slice %29#1[0, 0] [2, 2] [1, 1] : tensor<2x8xi64> to tensor<2x2xi64>
    ...
    %34 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%extracted_slice_5 : tensor<2x2xi64>) outs(%33 : tensor<2x2x8192x6144xf32>) {
    ^bb0(%in: i64, %out: f32):
      %73 = arith.cmpi slt, %in, %c0_i64 : i64
      %74 = arith.addi %in, %c8_i64 : i64
      %75 = arith.select %73, %74, %in : i64
    ...
    %expanded_6 = tensor.expand_shape %collapsed [[0], [1, 2, 3]] output_shape [2, 6144, 1, 1] : tensor<2x6144xf32> into tensor<2x6144x1x1xf32>
    %35 = tensor.empty() : tensor<2x1x1x6144xf32>
    %36 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%expanded_6 : tensor<2x6144x1x1xf32>) outs(%35 : tensor<2x1x1x6144xf32>) {
    ^bb0(%in: f32, %out: f32):
      linalg.yield %in : f32
    ...
    %collapsed_7 = tensor.collapse_shape %38 [[0], [1, 2, 3]] : tensor<2x6144x1x1xf32> into tensor<2x6144xf32>
    %expanded_8 = tensor.expand_shape %collapsed_7 [[0], [1, 2]] output_shape [2, 1, 6144] : tensor<2x6144xf32> into tensor<2x1x6144xf32>
    ...
    %collapsed_9 = tensor.collapse_shape %40 [[0], [1], [2, 3]] : tensor<2x6144x2x8192xf32> into tensor<2x6144x16384xf32>
    %41 = tensor.empty() : tensor<2x1x16384xf32>
    %42 = linalg.fill ins(%cst : f32) outs(%41 : tensor<2x1x16384xf32>) -> tensor<2x1x16384xf32>
    %43 = linalg.batch_matmul ins(%expanded_8, %collapsed_9 : tensor<2x1x6144xf32>, tensor<2x6144x16384xf32>) outs(%42 : tensor<2x1x16384xf32>) -> tensor<2x1x16384xf32>
    %expanded_10 = tensor.expand_shape %43 [[0], [1], [2, 3]] output_shape [2, 1, 2, 8192] : tensor<2x1x16384xf32> into tensor<2x1x2x8192xf32>
    %44 = tensor.empty() : tensor<2x2x8192x1xf32>
    %45 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%expanded_10 : tensor<2x1x2x8192xf32>) outs(%44 : tensor<2x2x8192x1xf32>) {
    ^bb0(%in: f32, %out: f32):
      linalg.yield %in : f32
    } -> tensor<2x2x8192x1xf32>
    %collapsed_11 = tensor.collapse_shape %45 [[0], [1], [2, 3]] : tensor<2x2x8192x1xf32> into tensor<2x2x8192xf32>
    %46 = tensor.empty() : tensor<2x2x8192xf32>
    %47 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel"]} ins(%collapsed_11 : tensor<2x2x8192xf32>) outs(%46 : tensor<2x2x8192xf32>) {
    ^bb0(%in: f32, %out: f32):
      %73 = arith.negf %in : f32
      %74 = math.exp %73 : f32
      %75 = arith.addf %74, %cst_1 : f32
    ...
    %48 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel"]} ins(%47, %collapsed_11 : tensor<2x2x8192xf32>, tensor<2x2x8192xf32>) outs(%46 : tensor<2x2x8192xf32>) {
    ^bb0(%in: f32, %in_23: f32, %out: f32):
      %73 = arith.mulf %in, %in_23 : f32
      linalg.yield %73 : f32
    } -> tensor<2x2x8192xf32>
    %49 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%extracted_slice_5 : tensor<2x2xi64>) outs(%33 : tensor<2x2x8192x6144xf32>) {
    ^bb0(%in: i64, %out: f32):
      %73 = arith.cmpi slt, %in, %c0_i64 : i64
      %74 = arith.addi %in, %c8_i64 : i64
      %75 = arith.select %73, %74, %in : i64
    ...
    %50 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d3, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%49 : tensor<2x2x8192x6144xf32>) outs(%39 : tensor<2x6144x2x8192xf32>) {
    ^bb0(%in: f32, %out: f32):
      linalg.yield %in : f32
    } -> tensor<2x6144x2x8192xf32>
    %collapsed_12 = tensor.collapse_shape %50 [[0], [1], [2, 3]] : tensor<2x6144x2x8192xf32> into tensor<2x6144x16384xf32>
    %51 = linalg.batch_matmul ins(%expanded_8, %collapsed_12 : tensor<2x1x6144xf32>, tensor<2x6144x16384xf32>) outs(%42 : tensor<2x1x16384xf32>) -> tensor<2x1x16384xf32>
    %expanded_13 = tensor.expand_shape %51 [[0], [1], [2, 3]] output_shape [2, 1, 2, 8192] : tensor<2x1x16384xf32> into tensor<2x1x2x8192xf32>
    %52 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%expanded_13 : tensor<2x1x2x8192xf32>) outs(%44 : tensor<2x2x8192x1xf32>) {
    ^bb0(%in: f32, %out: f32):
      linalg.yield %in : f32
    } -> tensor<2x2x8192x1xf32>
    %collapsed_14 = tensor.collapse_shape %52 [[0], [1], [2, 3]] : tensor<2x2x8192x1xf32> into tensor<2x2x8192xf32>
    %53 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel"]} ins(%48, %collapsed_14 : tensor<2x2x8192xf32>, tensor<2x2x8192xf32>) outs(%46 : tensor<2x2x8192xf32>) {
    ^bb0(%in: f32, %in_23: f32, %out: f32):
      %73 = arith.mulf %in, %in_23 : f32
      linalg.yield %73 : f32
    } -> tensor<2x2x8192xf32>
    %54 = tensor.empty() : tensor<2x2x6144x8192xf32>
    %55 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%extracted_slice_5 : tensor<2x2xi64>) outs(%54 : tensor<2x2x6144x8192xf32>) {
    ^bb0(%in: i64, %out: f32):
      %73 = arith.cmpi slt, %in, %c0_i64 : i64
      %74 = arith.addi %in, %c8_i64 : i64
      %75 = arith.select %73, %74, %in : i64
    ...
    %expanded_15 = tensor.expand_shape %53 [[0], [1], [2, 3]] output_shape [2, 2, 8192, 1] : tensor<2x2x8192xf32> into tensor<2x2x8192x1xf32>
    %56 = tensor.empty() : tensor<2x2x1x8192xf32>
    %57 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d3, d2)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%expanded_15 : tensor<2x2x8192x1xf32>) outs(%56 : tensor<2x2x1x8192xf32>) {
    ^bb0(%in: f32, %out: f32):
      linalg.yield %in : f32
    } -> tensor<2x2x1x8192xf32>
    %58 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d3, d2)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%57 : tensor<2x2x1x8192xf32>) outs(%44 : tensor<2x2x8192x1xf32>) {
    ^bb0(%in: f32, %out: f32):
      linalg.yield %in : f32
    } -> tensor<2x2x8192x1xf32>
    %collapsed_16 = tensor.collapse_shape %58 [[0, 1], [2, 3]] : tensor<2x2x8192x1xf32> into tensor<4x8192xf32>
    %expanded_17 = tensor.expand_shape %collapsed_16 [[0], [1, 2]] output_shape [4, 1, 8192] : tensor<4x8192xf32> into tensor<4x1x8192xf32>
    %59 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d3, d2)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%55 : tensor<2x2x6144x8192xf32>) outs(%33 : tensor<2x2x8192x6144xf32>) {
    ^bb0(%in: f32, %out: f32):
      linalg.yield %in : f32
    } -> tensor<2x2x8192x6144xf32>
    %collapsed_18 = tensor.collapse_shape %59 [[0, 1], [2], [3]] : tensor<2x2x8192x6144xf32> into tensor<4x8192x6144xf32>
    %60 = tensor.empty() : tensor<4x1x6144xf32>
    %61 = linalg.fill ins(%cst : f32) outs(%60 : tensor<4x1x6144xf32>) -> tensor<4x1x6144xf32>
    %62 = linalg.batch_matmul ins(%expanded_17, %collapsed_18 : tensor<4x1x8192xf32>, tensor<4x8192x6144xf32>) outs(%61 : tensor<4x1x6144xf32>) -> tensor<4x1x6144xf32>
    %expanded_19 = tensor.expand_shape %62 [[0, 1], [2], [3]] output_shape [2, 2, 1, 6144] : tensor<4x1x6144xf32> into tensor<2x2x1x6144xf32>
    %63 = tensor.empty() : tensor<2x2x6144x1xf32>
    %64 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d3, d2)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%expanded_19 : tensor<2x2x1x6144xf32>) outs(%63 : tensor<2x2x6144x1xf32>) {
    ^bb0(%in: f32, %out: f32):
      linalg.yield %in : f32
    } -> tensor<2x2x6144x1xf32>
    %collapsed_20 = tensor.collapse_shape %64 [[0], [1], [2, 3]] : tensor<2x2x6144x1xf32> into tensor<2x2x6144xf32>
    %expanded_21 = tensor.expand_shape %32 [[0], [1, 2]] output_shape [2, 2, 1] : tensor<2x2xf32> into tensor<2x2x1xf32>
    %65 = tensor.empty() : tensor<2x2x6144xf32>
    %66 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d1, 0)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel"]} ins(%expanded_21, %collapsed_20 : tensor<2x2x1xf32>, tensor<2x2x6144xf32>) outs(%65 : tensor<2x2x6144xf32>) {
    ^bb0(%in: f32, %in_23: f32, %out: f32):
      %73 = arith.mulf %in, %in_23 : f32
      linalg.yield %73 : f32
    } -> tensor<2x2x6144xf32>
    %67 = tensor.empty() : tensor<2x6144xf32>
    %68 = linalg.fill ins(%cst : f32) outs(%67 : tensor<2x6144xf32>) -> tensor<2x6144xf32>
    %69 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d2)>], iterator_types = ["parallel", "reduction", "parallel"]} ins(%66 : tensor<2x2x6144xf32>) outs(%68 : tensor<2x6144xf32>) {
    ^bb0(%in: f32, %out: f32):
      %73 = arith.addf %in, %out : f32
      linalg.yield %73 : f32
    } -> tensor<2x6144xf32>
    %expanded_22 = tensor.expand_shape %69 [[0], [1, 2]] output_shape [2, 1, 6144] : tensor<2x6144xf32> into tensor<2x1x6144xf32>
    %70 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, 0, d2)>, affine_map<(d0, d1, d2) -> (d0, 0, d2)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel"]} ins(%0, %expanded_22 : tensor<2x1x6144xf32>, tensor<2x1x6144xf32>) outs(%1 : tensor<2x1x6144xf32>) {
    ^bb0(%in: f32, %in_23: f32, %out: f32):
      %73 = arith.addf %in, %in_23 : f32
      linalg.yield %73 : f32
    } -> tensor<2x1x6144xf32>
    %71 = hal.tensor.barrier join(%70 : tensor<2x1x6144xf32>) => %arg2 : !hal.fence
    %72 = hal.tensor.export %71 : tensor<2x1x6144xf32> -> !hal.buffer_view
    util.return %72 : !hal.buffer_view
  }
  util.func public @decode_moe(%arg0: !hal.buffer_view) -> !hal.buffer_view attributes {iree.abi.stub} {
    %0 = util.null : !hal.fence
    %c-1_i32 = arith.constant -1 : i32
    %c0 = arith.constant 0 : index
    %device_0 = hal.devices.get %c0 : !hal.device
    %fence = hal.fence.create device(%device_0 : !hal.device) flags("None") : !hal.fence
    %1 = util.call @decode_moe$async(%arg0, %0, %fence) : (!hal.buffer_view, !hal.fence, !hal.fence) -> !hal.buffer_view
    %status = hal.fence.await until([%fence]) timeout_millis(%c-1_i32) : i32
    util.return %1 : !hal.buffer_view
  }
}